...
public interface UserRepository extends JpaRepository<User, Long> { // <(класс для кот. реализуем), (тип id)>
    ...
    @Query("Select u from User u left join fetch u.roles where u.username=:username") // Без этого отказывалось работать FetchType.LAZY в классе User, (type=Internal Server Error, status=500).
