...
public class UserGenerator {
    ...
    @Autowired
    public UserGenerator(RoleService roleService, PasswordEncoder passwordEncoder) {
    ...
    public User[] generateUsers(int count) {
        ...
        for (int i = 0; i < users.length; i++) {
            ...
            users[i] = new User(
                    login,
                    passwordEncoder.encode(Constants.DEFAULT_PASSWORD.get()), // "$2a$10$XtQgyP.gzuElotzb/lZDtORvqquxzcuZvifWc8NsCOFR6Lml1zpQS"; // defaultPassword "1"
                    name,
                    dateBirth,
                    address,
                    email);
